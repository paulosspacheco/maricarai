...
type
  ...
  TDmxScroller_Form_Lcl = class(TDmxScroller_Form_Lcl_attributes)
    ...
    protected procedure CreateForm(aaOwner:TComponent);override;
...
//    public procedure UpdateBuffers;override;
    ...
    protected procedure DoBeforeSetActiveTarget;override;
...
//    public function GetWidthChar(s:string):integer;Overload;
    ...
    public function TextWidthChar(AFont: TFont): Integer;
...
procedure TDmxScroller_Form_Lcl.CreateForm(aaOwner: TComponent);

   var aOwner : TScrollingWinControl;
...
begin
  ...
  aOwner := aaOwner as TScrollingWinControl ;
  if DMXFields = nil
  ...
  begin
    ...
    If Control is TMi_RadioGroup_LCL
    Then aTop := aTop + (HeightChar * (Control as TMi_RadioGroup_LCL).Items.Count-1)
    else aTop := aTop + HeightChar;
  end;
...
procedure TDmxScroller_Form_Lcl.DoBeforeSetActiveTarget;
begin
...
//Verifica se o componente fornecido possui uma relação db e se o conteúdo foi alterado
function TDmxScroller_Form_Lcl.isValueDbChanged(Sender: TComponent): Boolean;
  var
    tmp_Field: TField;
    dlink: TObject;
  begin
...
function TDmxScroller_Form_Lcl.TextWidthChar(AFont: TFont): Integer;
  var
    bmp: TBitmap;
begin
  Result := 0;
  ...
  try
    bmp.Canvas.Font.Assign(AFont);
    Result := bmp.Canvas.TextWidth('AbcXdFGHIJKLMNopqRstuVxZ0123489738') div 34;
//    WidthChar  := MyGetTextWidth('AbcXdFGHIJKLMNopqRstuVxZ32489738',(aOwner as TScrollingWinControl).Canvas.font)  div 32;
