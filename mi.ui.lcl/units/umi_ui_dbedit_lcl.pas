...
//      Private Var _EdittextAnt : String;
    ...
    public constructor Create(aOwner:TComponent;aUiDmxScroller : TUiDmxScroller);overload;
...
begin
  ...
  then with TUiDmxScroller do
       begin  //Excluir máscara porque Text não aceita números formatados
         s := DeleteMask(text,['0'..'9','-','+',showDecPt]);
         if pos(showDecPt ,s)<>0
         then begin
                text := Change_AnsiChar(s,showDecPt,DecPt);
              end
         else text := s;
  end;
...
constructor TMI_DbEdit_LCL.Create(aOwner: TComponent;aUiDmxScroller: TUiDmxScroller);
begin
  Create(AOwner);
...
procedure TMI_DbEdit_LCL.SetUiDmxScroller(aUiDmxScroller: TUiDmxScroller);
begin
...
begin
  ...
  then Try
          ...
          if (upcase(DmxFieldRec.CharShowPassword) = UiDmxScroller.CharShowPassword)
...
//          onKeyDown := DoOnKeyDown;
          if DmxFieldRec^.IsNumber
          then with DmxFieldRec^,UiDmxScroller do
               begin
                 onKeyPress := DoEditNumberKeyPress;

                 _DisplayFormat := Get_MaskEdit_LCL(Template_org, _OkMask);
                 ...
                 with DmxFieldRec^,UiDmxScroller do
...
//             if (pos(showDecPt ,s)<>0) and (showDecPt<>DecPt)
//             then begin
...
////     showMessage(s);
//
//     //If DmxFieldRec.TypeCode  in [FldData,fldLData,fld_LData]
//     //Then Begin {$REGION '--->'}
//     //       while Pos('/',s) <> 0
//     //       do DELETE(s,Pos('/',s),1);
//     //
...
function TMI_DbEdit_LCL.GetHelpCtx_Hint: AnsiString; // Documento do objeto.
  ...
  //begin
  //   Result := '';//(DmxFieldRec.owner as TUiDmxScroller).GetHelpCtx_Porque;
  //end;
  //
  //function TMI_DbEdit_LCL.GetHelpCtx_Onde: AnsiString;
  //begin
  //  Result := '';//(DmxFieldRec.owner as TUiDmxScroller).GetHelpCtx_Onde;
  //end;
  //
  //function TMI_DbEdit_LCL.GetHelpCtx_Como: AnsiString;
  //begin
  //  Result := '';//(DmxFieldRec.owner as TUiDmxScroller).GetHelpCtx_Como;
  //end;
  //
  //function TMI_DbEdit_LCL.GetHelpCtx_Quais: AnsiString;
  //begin
  //  Result := '';//(DmxFieldRec.owner as TUiDmxScroller).GetHelpCtx_Quais;
  //end;
  //
...
{$REGION '---> Customizações da classe TMI_DbEdit_LCL para implementar a  Interface IInputText <---'}
  ...
  //function TMI_DbEdit_LCL.Getvidis_OnEnter: Boolean;
  //begin
  //
  //end;
  //
  //function TMI_DbEdit_LCL.Getvidis_OnExit: Boolean;
  //begin
  //
  //end;
...
//  end;
  ...
  //END;
  //
  //procedure TMI_DbEdit_LCL.Setvidis_OnEnter(avidis_OnEnter: Boolean);
  //begin
  //  DmxFieldRec.vidis_OnEnter := avidis_OnEnter;
  //end;
  //
  //procedure TMI_DbEdit_LCL.Setvidis_OnExit(avidis_OnExit: Boolean);
  //begin
  //  DmxFieldRec.vidis_OnExit := avidis_OnExit;
  //end;
      ...
      MaxLength := Espaco;
       ...
       Then Begin
               ...
               begin
                 ...
                 s := DeleteMask(s,['0'..'9',DecPt,showDecPt]);
                 iaux := pos(showDecPt,s);
                 if (Iaux<>0) and (DecPt<>showDecPt)
                 then begin
                        system.delete(s,iaux,1);
                        system.insert(DecPt,s,iaux);
                      end;
                 Text := s;
