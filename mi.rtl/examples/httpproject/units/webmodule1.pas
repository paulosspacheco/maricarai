...
type
  ...
  TFPWebModule1 = class
    ...
    procedure TFPWebAction0Request(Sender: TObject; ARequest: TRequest;
      AResponse: TResponse; var Handled: Boolean); private   Params: tObjectss.TMiStringList;

  protected
     Procedure DoAction0RequestTag1 (Sender: TObject; Const TagString:String;TagParams:TStringList; Out ReplaceText: String);//for tags with parameters support
  public
...
begin
  //ModuleTemplate é uma propriedade global do módulo web
  //Para usar a propriedade Template da ação da web atual (que é visível em
  //o inspetor de objeto para cada ação), use (Remetente como TFPWebAction).Template.FileName := 'mytemplate1.html'; e assim por diante.  ModuleTemplate.FileName := './templates/mytemplate1.html';

  ModuleTemplate.AllowTagParams := true;
  ModuleTemplate.StartDelimiter := '<!--#';
  ModuleTemplate.EndDelimiter := '#-->';
  ModuleTemplate.OnReplaceTag := @DoAction0RequestTag1;
  AResponse.Content := ModuleTemplate.GetContent;

  Handled := true;
end;

procedure TFPWebModule1.DoAction0RequestTag1(Sender: TObject;
