...
begin
  DockMaster.ShowControl('form_xtable',true);
end;
  ...
  case AnsiIndexStr(UpperCase(aName),
     ...
     0 : begin
           //Obrigatório que o módulo de dados seja criado antes do fromulário
            dm_xtable := Tdm_xtable.Create(AControl);

           // Cria o formulário
           AControl := TFormDock.GetForm(self,tform_xtable,aName,DoDisableAutoSizing);

           //Conecta o modulo de dados ao formulário
           (AControl as Tform_xtable).SetDataModule(dm_xtable);
         end;
     1 : begin
           AControl := nil;
         end;
