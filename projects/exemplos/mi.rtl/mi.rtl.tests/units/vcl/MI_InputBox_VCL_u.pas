...
//    OKMaiuscula := false;
    ...
    If aTypeFld  in [FldDateTime,FldDateTime,FldDateTime]
    Then  Begin
            OkMask := true;
            Result  := '99/99/00;0;_';
            aLength_Buffer := 6;
...
//           oKMaiuscula := true;
        ...
        Else If IsNumber_Real(aTypeFld)
             Then Begin
                    If aTemplate[i] = DecimalSeparator 
                    Then Result := Result + ',';
                    OKSeparador := true;
...
//                        MI_ComboBoxEx1.Value  := Buffer;
        ...
        { fldInteger            , //=  'I'; } {< integer Field }
          ...
          fldDouble            , //=   'R';  {< real number Field  (uses TRealNum) }
          fldDoublePositive   ,
          FldDateTime,FldDateTime,FldDateTime
          :
                  Begin
                    {$REGION '---> TMaskEdit.Create()'}

                      If Result_TypeFld  in [FldDateTime,FldDateTime,FldDateTime]
                      Then Begin {$REGION '--->'}
                             while Pos('/',buffer) <> 0
...
//                      E.Parent    := form;
                      ...
                      then Begin
              {$REGION ' ---> 2013/04/03 - Tarefa: A mascara de campos do tipo senha não está funcionando na conversão de mascaras de tv32 para VCL.'}
     ...
     Begin
         ...
         Result_TypeFld := TypeFld(Template,Size_TypeFld);
          Case Result_TypeFld  of
            ...
            FldDateTime            : Result := Db_Datas.DateToStr(TypeData(Buff),DateMask_dd_mm_yy);
            FldDateTime,
            FldDateTime          : Result := DateToStr(LONGINT(Buff),DateMask_dd_mm_yy);
            fldLHora,
            fld_LHora          : Result := HourToStr(LONGINT(Buff),HourMask_hh_nn,False);
  ...
  //                                  Result := FloatToStr(Real(Buff)*100);
     ...
     Begin
          ...
          Case TypeFld(Template,Size_TypeFld) of
            ...
            FldDateTime            : TypeData(Buff) := StrToDate(VarBuff,DateMask_dd_mm_yy)^;
            FldDateTime,
            FldDateTime          : Longint(Buff) := PackDate(VarBuff,DateMask_dd_mm_yy);
            fldLHora,
            fld_LHora          : Longint(Buff) := StrToHour(VarBuff,HourMask_hh_nn);// Falta implementar esta funcao.
