...
uses
  ...
  ,mi.rtl.Types
  ,Types
  ,mi.rtl.objects.methods.pageproducer.test, Unit2
  ,mi.rtl.all
...
type
  ...
  {: - O formulário **@name** é usado para testar o pacote pacote
       mi.rtl}
  TForm1 = class(TForm)
      Action_TestGetFieldsKeys: TAction;
      Action_Test_FirtRec_AddRec_NextRec_PrevRec_LastRec_json: TAction;
      Action_Test_FirtRec_AddRec_NextRec_PrevRec_LastRec: TAction;
...
//    MenuItem_Tobjectss_logs: TMenuItem;
    ...
    procedure Action_TestGetFieldsKeysExecute(Sender: TObject);
    procedure Action_Test_FirtRec_AddRec_NextRec_PrevRec_LastRecExecute(
      Sender: TObject);
    procedure Action_Test_FirtRec_AddRec_NextRec_PrevRec_LastRec_jsonExecute(
      Sender: TObject);
...
{: A procedure @name demonstra o uso da classe  **TObjectss.TFileStream** quando o registro 0 (zero) do arquivo
  ...
  begin
    ...
    with TMi_rtl do
      if InputPassword('Usuario e senha?',username, senha)
      then begin
              Alert('Password ','Usuário= '+UserName +'e senha= '+Senha)
           end
      else Alert('Password','Senha não informada');


    with TMi_rtl do
      if InputPassword('Password',senha)
      then Alert('Password','A senha digitada é: '+senha)
      else Alert('Password','Senha não informada');
  ...
  // Retorna todos os arquivos da pasta e subpastas
     ...
     //Then FindFilesAll(ExtractFileDir(Edit1.Text),aExt,FileAttrs,true ,ListFiles,true );
     FindFilesAll(ExtractFileDir(Edit1.Text),aExt,FileAttrs,true ,ListFiles,true );
  ...
  procedure TForm1.Action_Test_FirtRec_AddRec_NextRec_PrevRec_LastRecExecute(  Sender: TObject);
  begin
    TDataModule1.Test_FirtRec_AddRec_NextRec_PrevRec_LastRec;
    ShowMessage('O teste foi impresso no arquivo de log: mi.rtl.tests.rtl.log');
  end;

  procedure TForm1.Action_Test_FirtRec_AddRec_NextRec_PrevRec_LastRec_jsonExecute(  Sender: TObject);
  begin
    TDataModule1.Test_FirtRec_AddRec_NextRec_PrevRec_LastRec_Json;
    ShowMessage('O teste foi impresso no arquivo de log: mi.rtl.tests.rtl.log');
  end;

  procedure TForm1.Action_TestGetFieldsKeysExecute(Sender: TObject);
    var
  ...
  begin
    ...
    try
      ...
      Keys := Tmi_rtl.getFieldsKeys(QueryFields, Values);
