...
unit __unit_dm_connections__;
{:< A unit **@name** implementa o datamoule __DM_Connections__ configurado para
...
type
  {: O registro **@name** é usado para transferir os parâmetros entre os métodos
     do DataModule __DM_Connections__.
...
type
  ...
  T__DM_Connections__ = class(TDataModule)
...
var
  __DM_Connections__: T__DM_Connections__;
...
procedure T__DM_Connections__.DmxScroller_Form1CloseQuery(  aDmxScroller: TUiDmxScroller; var CanClose: boolean);
begin
...
procedure T__DM_Connections__.DataModuleDestroy(Sender: TObject);
begin
  if Connection
...
function T__DM_Connections__.GetForm_Connections(out ConnectorType
                                                    ,HostName
                                                    ,UserName
...
function T__DM_Connections__.GetParametros_Conexao(aFileName: String;
...
procedure T__DM_Connections__.SetParametros_Conexao(aFileName: String;  var aParam: TParam);
begin
  With aParam do
  try
    _Inifile := TINIFile.Create(aFileName);
...
procedure T__DM_Connections__.SQLConnector1Login(Sender: TObject; Username, Password: string);
begin
...
procedure T__DM_Connections__.SetConnection(a_Connection: Boolean);
...
function T__DM_Connections__.GetConnection(): Boolean;
begin
