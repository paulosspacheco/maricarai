...
unit unit_dm;
{:< A unit **@name** implementa o datamoule DM configurado para
       ...
       - O evento **OnCreate** do forme principal deve executar o seguinte código:
         ...
         ```pascal
            ...
            then begin
                   ...
                   else .Connection  := false; //Fecha a conexão com o banco de dados
                 end;
...
procedure TDM.DmxScroller_Form1CloseQuery(  aDmxScroller: TUiDmxScroller; var CanClose: boolean);
begin
...
function TDM.GetForm_Connections(out ConnectorType
                                                    ,HostName
                                                    ,UserName
...
function TDM.GetParametros_Conexao(aFileName: String;
...
procedure TDM.SetParametros_Conexao(aFileName: String;  var aParam: TParam);
begin
  With aParam do
  try
    _Inifile := TINIFile.Create(aFileName);
...
procedure TDM.SQLConnector1Login(Sender: TObject; Username, Password: string);
begin
...
procedure TDM.SetConnection(a_Connection: Boolean);
...
function TDM.GetConnection(): Boolean;
begin
