...
uses
  ...
  Menus, ComCtrls, StdCtrls, Buttons, strUtils, AnchorDockPanel, AnchorDocking,
  XMLPropStorage, AnchorDockOptionsDlg, mi.rtl.all, mi_rtl_ui_Dmxscroller,
  mi_rtl_ui_dmxscroller_form, uMi_lcl_inputbox, uMi_lcl_ui_Form,
  udm_connections, uFormDock, uoperadores, uform_operadores, uhospitais,
  uform_hospitais, umedicos, uservico_de_agendas, uexpediente_do_medico_data,
  uexpediente_do_medico_horas, uform_medicos, uform_servico_de_agendas,
  uform_expediente_do_medico_data, uform_expediente_do_medico_horas
  ;
...
type
  ...
  Tform_main = class(TForm)
    ...
    Action_expediente_do_medico_horas: TAction;
    Action_expediente_do_medico_data: TAction;
    Action_servico_de_agendas: TAction;
    Action_medicos: TAction;
    Action_hospitais: TAction;
    ...
    procedure Action_expediente_do_medico_dataExecute(Sender: TObject);
    procedure Action_expediente_do_medico_horasExecute(Sender: TObject);
    procedure Action_helpExecute(Sender: TObject);
    procedure Action_hospitaisExecute(Sender: TObject);
    procedure Action_medicosExecute(Sender: TObject);
    procedure Action_operadoresExecute(Sender: TObject);
    procedure Action_servico_de_agendasExecute(Sender: TObject);
...
procedure Tform_main.Action_hospitaisExecute(Sender: TObject);
begin
...
procedure Tform_main.Action_expediente_do_medico_dataExecute(Sender: TObject);
begin
  DockMaster.ShowControl('form_expediente_do_medico_data',true);
end;

procedure Tform_main.Action_expediente_do_medico_horasExecute(Sender: TObject);
begin
  DockMaster.ShowControl('form_expediente_do_medico_horas',true);
end;

procedure Tform_main.Action_helpExecute(Sender: TObject);
begin
  Mi_lcl_ui_Form1.ShowHtml('/home/paulosspacheco/Área de Trabalho/LAZARUS/maricarai_fixe-3.1/projects/ph.app.br/docs/index.html');
end;
...
procedure Tform_main.Action_servico_de_agendasExecute(Sender: TObject);
begin
  DockMaster.ShowControl('form_servico_de_agendas',true);
end;
...
procedure Tform_main.DockMasterCreateControl(Sender: TObject;  aName: string; var AControl: TControl; DoDisableAutoSizing: boolean);
  ...
  function GetFormLocal: TControl;
  begin
...
begin
  case AnsiIndexStr(UpperCase(aName),
       [UpperCase('form_operadores'), //0
        UpperCase('form_hospitais'),  //1
        UpperCase('form_medicos'),//2
        UpperCase('form_servico_de_agendas'), //3
        UpperCase('form_expediente_do_medico_data'),//4
        UpperCase('form_expediente_do_medico_horas'), //5
        UpperCase('6'),
        UpperCase('7'),
        UpperCase('8'),
        UpperCase('9'),
     ...
     0 : begin //form_operadores
          AControl := GetFormLocal;
          if Assigned(AControl)
          then exit
          else begin
                 AControl := TFormDock.GetForm(tform_operadores,aName,DoDisableAutoSizing);
                 operadores :=Toperadores.Create(AControl);
                 if Assigned(AControl)
                 Then (AControl as Tform_operadores).SetDataModule(operadores);
               end;
         end;
     ...
     3 : begin //Tform_servico_de_agendas
           AControl := GetFormLocal;
           if Assigned(AControl)
           then exit
           else begin
                  AControl := TFormDock.GetForm(Tform_servico_de_agendas,aName,DoDisableAutoSizing);
                  (AControl as Tform_servico_de_agendas).SetDataModule(Tservico_de_agendas.Create(AControl));
               end;
         end;
     4 : begin   //Tform_expediente_do_medico_data
           AControl := GetFormLocal;
           if Assigned(AControl)
           then exit
           else begin
                  AControl := TFormDock.GetForm(Tform_expediente_do_medico_data,aName,DoDisableAutoSizing);
                  (AControl as Tform_expediente_do_medico_data).SetDataModule(TExpediente_do_medico_data.Create(AControl));
               end;

         end;

     5 : begin  //Tform_expediente_do_medico_horas
           AControl := GetFormLocal;
           if Assigned(AControl)
           then exit
           else begin
                  AControl := TFormDock.GetForm(Tform_expediente_do_medico_horas,aName,DoDisableAutoSizing);
                  (AControl as Tform_expediente_do_medico_horas).SetDataModule(TExpediente_do_medico_horas.Create(AControl));
               end;
         end;
