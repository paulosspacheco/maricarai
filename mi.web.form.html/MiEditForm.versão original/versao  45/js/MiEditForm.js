...
export class MiEditForm extends MiConsts {
    ...
    constructor(baseUrl,formId, keyFields) {
        super(); // Chama o construtor da classe pai

        if (arguments.length === 3) {
            // Três parâmetros fornecidos: baseUrl, formId, keyFields
            this.baseUrl = baseUrl;
            this.formId = formId;
            this.keyFields = keyFields;
          } else if (arguments.length === 2) {
            // Dois parâmetros fornecidos: formId, keyFields (sem baseUrl)
            this.baseUrl = this.GetBaseUrl; // Valor padrão para baseUrl
            this.formId = baseUrl;  // Neste caso, o primeiro argumento é formId
            this.keyFields = formId; // O segundo argumento é keyFields
          } else {
            throw new Error("Número incorreto de argumentos.");
          }
      

        // let url = `http://192.168.15.2:8080/Tmi_rtl_web_module/${action}`;                
        // let url = `http://45.160.125.12:8080/Tmi_rtl_web_module/${action}`;                
        // Armazena a URL base no constructor
        // Concatena baseUrl e formId de maneira simples
        // this.baseUrl = baseUrl + '/' + formId;       
        this.baseUrl = `${baseUrl}/${formId}`;
    ...
    async sendRequest(action, queryParams, jsonData, method) {               
        ...
        try {
            ...
            // Constrói a URL usando a base URL do construtor
...
return; //está com problema em getParam
    ...
    static GetBaseUrl(){      
      return MiConsts.hostName+':'+MiConsts.port;  
    }
