...
export class MiEditForm extends MiConsts {
    ...
    //     // Criar uma nova instância do alerta dentro do método
    //     // const alert = new Alert('mtWarning'); // A classe Alert deve estar acessível no contexto
    //     //const alert = new Alert('mtError'); // A classe Alert deve estar acessível no contexto
    //     const alert = new Alert(type);
        
    //     // Exibir a mensagem
    //     alert.show(aMsg); // Chamando o método show da instância alert
    
    //     // Aguardar o usuário clicar em "OK" ou fechar o alerta
    //     await new Promise((resolve) => {
    //         alert.okButton.addEventListener('click', () => {
    //             alert.close(); // Fechar o alerta
    //             resolve(); // Resolva a promessa quando o botão OK for clicado
    //         });
    //     });
    
    //     // Remover a referência ao alerta, se necessário
    //     alert.overlay.remove(); // Remove o overlay do DOM
    //     // alert.close; não precisa porque alert é local
    // }
    ...
    /**
     * Método messageBox que chama o alert.messageBox e garante que o alerta seja removido após o uso.
     ...
     */
    async messageBox(message, title = 'Alerta', dialogType = 'mtInformation', buttons = ['mbOK']) {
