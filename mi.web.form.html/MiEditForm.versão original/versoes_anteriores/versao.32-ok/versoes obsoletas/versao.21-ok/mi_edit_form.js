...
export class MiEditForm extends MiConsts {
    ...
    constructor(formId, keyFields) {
        ...
        try {
            ...
            this.alert.show('Erro ao normalizar campos de formulário:', error);
        }
    ...
    init(){
        ...
        try {
            ...
            this.alert.show('Erro ao verificar disponibilidade da API:', error);
        }        
        this.updateCommands();
    ...
    redraw() {
        ...
        for (const actionName in this.actionList) {
            ...
            if (button) {
                ...
                this.alert.show(`Botão não encontrado: ${action.name}`);
            }
        }
    ...
    async checkApiAvailability() {
        try {
            ...
            if (status === 200) {
                ...
                this.alert.show(`Servidor não está disponível!`);                
            }
            //this.redraw();
        } catch (error) {
            this.alert.show(`Erro ao verificar disponibilidade da API: ${error.message}`);
            this.alert.show(`Erro ao verificar disponibilidade da API: ${error.message}`);
            this.setState(MiConsts.Mb_St_Active, false); // Desabilita o estado ativo
        }
    }
    ...
    setFieldValue(fieldName, value) {
        ...
        if (field) {
            ...
            this.alert.show(`Campo com ID ${fieldName} não encontrado.`);
        }
    }

    updateForm(data) {
        if (!data || typeof data !== 'object') {
            this.alert.show('Dados para atualizar o formulário são nulos ou inválidos:', data);
            return;
        }
    ...
    buildQueryParams() {
        ...
        const keyValues = lowerCaseKeyFields.map(lowerCaseField => {
            ...
            if (matchingField) {
                ...
                this.alert.show(`Campo correspondente não encontrado para: ${lowerCaseField}`);
        ...
        if (keyValues.trim() === '') {
            this.alert.show('Atenção: Valores de campos-chave estão vazios.');
        }
    ...
    saveWorkingData() {
        ...
        if (this.isDataSetActive) {
            ...
            if (this.workingData && this.workingDataOld) {
                ...
                // this.alert.show('Erro: workingData ou workingDataOld não são válidos.'); // Log de erro se os dados não são válidos
                this.alert.show('Erro: workingData ou workingDataOld não são válidos.');
            }
        } else {
            // this.alert.show('Aviso: O conjunto de dados não está ativo. Nenhum dado foi salvo.'); // Aviso se o dataset não estiver ativo
            this.alert.show('Aviso: O conjunto de dados não está ativo. Nenhum dado foi salvo.');
        }
    }
    ...
    restoreWorkingData() {
        if (this.isDataSetActive) {
            if (this.workingData && this.wWorkingData && this.workingDataOld && this.wWorkingDataOld) {
                ...
                this.alert.show('Erro: Dados de trabalho não estão definidos corretamente.');
            }
        } else {
            this.alert.show('Aviso: O conjunto de dados não está ativo. Nenhum dado foi restaurado.');
        }
    }
    ...
    updateWorkingDataFromServer(serverData) {
        ...
        if (serverData && typeof serverData === 'object') {
            ...
            this.alert.show('Erro: serverData não é um objeto válido.');
        ...
        if (this.workingData && typeof this.workingData === 'object') {
            ...
            this.alert.show('Aviso: workingData não está definido ou não é um objeto antes de fazer uma cópia.');
        ...
        if (this.workingData && typeof this.workingData === 'object') {
            ...
            this.alert.show('Erro: workingData não é um objeto válido após atualização com serverData.');
        }
    ...
    cancel() {
        try {
            ...
            this.alert.show(`Erro ao cancelar a operação: ${error.message}`);
        }
    }    
    ...
    async newRecord() {
        try {
            ...
            this.alert.show(`Erro ao criar novo registro: ${error.message}`);
        }
    }
        ...
        try {
            ...
            if (status === 201 || status === 200) {
                if (responseData) {
                    ...
                    this.setState(MiConsts.Mb_St_Insert, false);
                    this.setState(MiConsts.Mb_St_Edit, true);                                    
                    this.alert.show('Registro adicionado com sucesso.');
                } else {
                    this.alert.show('Registro adicionado, mas sem dados retornados.');
                }
            } else {
                this.alert.show(`Erro: ${JSON.stringify(responseData, null, 2)}`);
            }
        } catch (error) {
            this.alert.show(`Erro ao adicionar registro: ${error.message}`);
        }
    }
        ...
        try {
            ...
            if (status === 200 && data) {
                ...
                if (data && data.message) {
                    this.alert.show(`Erro: ${data.message}`);
                } else {
                    this.alert.show('Registro não encontrado.');
                }
            } else {
                // Outros casos de erro
                console.log(`Erro inesperado: status ${status}`);
                this.alert.show('Erro inesperado ao localizar registro.');
            }
        } catch (error) {
            // Erro de requisição ou de rede
            this.alert.show(`Erro ao localizar registro: ${error.message}`);
        }
    }     
    ...
    async PutRecord() {
        try {
            ...
            if (status === 200) {
                ...
                this.setState(MiConsts.Mb_St_Insert, false);
                this.setState(MiConsts.Mb_St_Edit, true);                
                this.alert.show('Registro atualizado com sucesso.');                
            } else {
                this.alert.show('Erro ao atualizar registro.');
            }
        } catch (error) {
            this.alert.show(`Erro ao atualizar registro: ${error.message}`);
        }
    }
    ...
    async deleteRecord() {
        try {
            ...
            if (status === 200) {
                ...
                this.alert.show('Registro excluído com sucesso.');
            } else if (status === 500) {
    ...
    async goBof() {
        try {
            ...
            if (status === 200) {
                ...
                this.alert.show(`Erro ao ir para o primeiro registro: ${data.message}`);
                return false; // Retorna false em caso de falha
            }
        } catch (error) {
            this.alert.show(`Erro ao ir para o primeiro registro: ${error.message}`);
        ...
        try {
            if (!this.eof) {
                ...
                if (status === 200 && data) {
                    ...
                    this.bof = false; 
                    this.eof = true;
                    this.alert.show(`Erro ao obter próximo registro: ${data.message}`);
                    this.updateCommands(); 
                    return false;
                }
            }
            ...
            this.alert.show(`Erro ao obter próximo registro: ${error.message}`);
            return false;
        }
    }
        ...
        try {
            if (!this.bof) {
                ...
                if (status === 200 && data) {                    
                    ...
                    this.bof = true;  
                    this.eof = false;
                    this.alert.show(`Erro ao obter registro anterior: ${data.message}`);
                    this.updateCommands(); 
                    return false; 
                }
            } 
            ...
            this.alert.show(`Erro ao obter registro anterior: ${error.message}`);
            return false; 
        }
    }
    ...
    async goEof() {
        try {
            ...
            if (status === 200) {
                ...
                this.alert.show(`Erro ao ir para o último registro: ${data.message}`);
                return false; // Retorna false em caso de falha
            }
        } catch (error) {
            this.alert.show(`Erro ao ir para o último registro: ${error.message}`);
            return false; // Retorna false em caso de exceção
        }
    }
  ...
  // Método para alterar o estado baseado em alterações no campo
    ...
    async handleAction(action) {
        switch (action) {
            ...
            default:
                this.alert.show(`Ação ${action} não reconhecida.`);
        }
    }
    ...
    initializeEventListeners() {
        ...
        const actions = ['CmHealthCheck', 'CmNewRecord', 'CmAddRecord', 'CmPutRecord', 'CmLocate', 'CmDeleteRecord', 'CmRefresh', 'CmCancel'];
