class CRUDHandler {
    ...
    async addRecord() {
        try {
            ...
            const { status, responseData } = await this.sendRequest('CmAddRecord', data, 'POST');
            ...
            if (status === 201) {
                ...
                alert(`Erro ao adicionar registro. Status: ${status}. Detalhes: ${JSON.stringify(responseData, null, 2)}`);
            }
        } catch (error) {
