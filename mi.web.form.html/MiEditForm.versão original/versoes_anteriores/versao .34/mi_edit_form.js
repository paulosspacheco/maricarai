...
import { TMaskEdit } from './maskedit.js';
...
export class MiEditForm extends MiConsts {
    ...
    constructor(formId, keyFields) {
        ...
        // Inicializa os listeners de inputs assim que a página estiver carregada
    ...
    initializeMasks() {
        // Seleciona todos os inputs dentro do formulário que têm o atributo 'data-mask'
        const inputsWithMask = this.form.querySelectorAll('input[data-mask]');

        inputsWithMask.forEach(input => {
            const mask = input.getAttribute('data-mask');
            const maskType = input.getAttribute('data-mask-type');

            // Inicializa a classe TMaskEdit para cada input com máscara
  ...
  // Método para alterar o estado baseado em alterações no campo
    ...
    onEnterField() {        
        ...
        if (this.currentField && this.currentField.value !== undefined) {
    ...
    setCurrentField(aCurrentField) {
        // Salva o campo anterior
        this.currentFieldOld = { ...this._currentField }; // Verifique se _currentField está definido
