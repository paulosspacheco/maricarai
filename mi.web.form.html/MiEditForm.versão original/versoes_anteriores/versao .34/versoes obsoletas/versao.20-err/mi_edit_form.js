...
export class MiEditForm extends MiConsts {
    ...
    constructor(formId, keyFields) {
        ...
        try {
            ...
            console.error('Erro ao normalizar campos de formulário:', error);
        }
    ...
    destroy() {
        ...
        // Limpa referências
        this.form = null; // Libera a referência ao formulário
        this.isActive = false; // Atualiza o estado
        this.workingData = null; // Buffer corrente
        this.wWorkingData = null; // Cópia dos dados atuais
        this.workingDataOld = null; // Buffer anterior        
        this.wWorkingDataOld = null; // Cópia dos dados antigos        
        console.log('Recursos liberados e estado limpo.');
    }
    ...
    redraw() {
        ...
        for (const actionName in this.actionList) {
            ...
            if (button) {
                ...
                console.error(`Botão não encontrado: ${action.name}`);
            }
        }
    ...
    async checkApiAvailability() {
        try {
            ...
            if (status === 200) {
                ...
                // API indisponível
                this.alert.show('Servidor não está ativo!');
    ...
    updateCommands() {
        ...
        if (this.state & MiConsts.Mb_St_Insert) { // Modo de inserção
            ...
            if (this.appending && this.recordAltered) {
                ...
                this.enableCommands([
                    'CmNewRecord', 'CmGoBof', 'CmNextRecord', 'CmPrevRecord', 'CmGoEof']
                );
            }
    ...
    saveWorkingData() {
        ...
        if (this.isDataSetActive) {
            ...
            if (this.workingData && this.workingDataOld) {
                ...
                console.error('Erro: workingData ou workingDataOld não são válidos.'); // Log de erro se os dados não são válidos
            }
        } else {
            console.warn('Aviso: O conjunto de dados não está ativo. Nenhum dado foi salvo.'); // Aviso se o dataset não estiver ativo
        }
    }
    ...
    restoreWorkingData() {
        if (this.isDataSetActive) {
            if (this.workingData && this.wWorkingData && this.workingDataOld && this.wWorkingDataOld) {
                ...
                console.error('Erro: Dados de trabalho não estão definidos corretamente.');
            }
        } else {
            console.warn('Aviso: O conjunto de dados não está ativo. Nenhum dado foi restaurado.');
        }
    }
    ...
    updateWorkingDataFromServer(serverData) {
        ...
        if (serverData && typeof serverData === 'object') {
            ...
            console.error('Erro: serverData não é um objeto válido.');
        ...
        if (this.workingData && typeof this.workingData === 'object') {
            ...
            console.warn('Aviso: workingData não está definido ou não é um objeto antes de fazer uma cópia.');
        ...
        if (this.workingData && typeof this.workingData === 'object') {
            ...
            console.error('Erro: workingData não é um objeto válido após atualização com serverData.');
        }
    ...
    cancel() {
        try {
            ...
            console.error(`Erro ao cancelar a operação: ${error.message}`);
        }
    }    
    ...
    async refresh() {
        try {
            this.cancel();
            // Verifica se está no modo insert
            if (this.appending) {
                this.clearForm();                
                this.init();              
            } else {
    ...
    async deleteRecord() {
        try {
            ...
            if (status === 200) {
                ...
                console.error(`Erro interno no servidor: ${errorMsg}`);
            } else if (status === 404) {
                ...
                this.alert.show(errorMsg);
                console.error(`Erro inesperado: ${errorMsg}`);
            }
        } catch (error) {
  ...
  // Método para alterar o estado baseado em alterações no campo
    ...
    async handleAction(action) {
        switch (action) {
            ...
            default:
                console.error(`Ação ${action} não reconhecida.`);
        }
    }
    ...
    disposeInputListeners() {
        // Remove o evento de entrada de cada campo
        this.campos.forEach(campo => {
            campo.removeEventListener('input', campo.inputHandler);
            // Opcional: Limpa a referência ao manipulador
            campo.inputHandler = null;
        });
        this.campos = []; // Limpa a lista de campos
    }
    ...
    disposeEventListeners() {
        // Remove os ouvintes de eventos dos botões de ação
        this.actionButtons.forEach(({ button, handler }) => {
            button.removeEventListener('click', handler);
        });
        this.actionButtons = []; // Limpa a lista de botões de ação

        // Remove os ouvintes de eventos dos botões de navegação
        this.navigationButtons.forEach(({ button, handler }) => {
            button.removeEventListener('click', handler);
        });
        this.navigationButtons = []; // Limpa a lista de botões de navegação
    }    
