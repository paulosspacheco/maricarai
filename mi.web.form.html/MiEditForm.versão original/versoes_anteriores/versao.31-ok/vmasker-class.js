    ...
    for (let i = 0; i < outputLength; i++) {
      if (index >= values.length) {
        if (patternChars.length == charsValues.length) {
          ...
          return VMasker.addPlaceholdersToOutput(output, i, placeholder).join("");
        } else {
          break;
        }
      } else {
        if ((output[i] === VMasker.DIGIT && values[index].match(/[0-9]/)) ||
          (output[i] === VMasker.ALPHA && values[index].match(/[a-zA-Z]/)) ||
          (output[i] === VMasker.ALPHANUM && values[index].match(/[0-9a-zA-Z]/))) {
          output[i] = values[index++];
        } else if (output[i] === VMasker.DIGIT || output[i] === VMasker.ALPHA || output[i] === VMasker.ALPHANUM) {
