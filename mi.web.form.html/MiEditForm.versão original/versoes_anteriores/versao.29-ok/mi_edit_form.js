...
export class MiEditForm extends MiConsts {
    ...
    async checkApiAvailability() {
        try {
            ...
            if (status === 200) {
                ...
                this.showMessage('mtError',`Servidor não está disponível!`);                
            }
            //this.redraw();
        } catch (error) {
    ...
    setFieldValue(fieldName, value) {
        ...
        if (field) {
            ...
            this.showMessage('mtError',`Campo com ID ${fieldName} não encontrado.`);
        }
    }
    ...
    restoreWorkingData() {
        if (this.isDataSetActive) {
            ...
            this.showMessage('mtError','Aviso: O conjunto de dados não está ativo. Nenhum dado foi restaurado.');
        }
    }
    ...
    alert(message) {
        try {
            const dialogs = new Dialogs('mtWarning',['OK']); // Instancia a classe com o botão "OK"
            return dialogs.showMessage('mtWarning', message); // Chama o método showMessage
        } catch (error) {
        ...
        try {
            ...
            if (status === 200 && data) {
                ...
                if (data && data.message) {
                    this.showMessage('mtError',`Erro: ${data.message}`);
                } else {
                ...
                this.showMessage('mtError','Erro inesperado ao localizar registro. status ');
            }
        } catch (error) {
    ...
    async PutRecord() {
        try {
            ...
            if (status === 200) {
                ...
                this.showMessage('mtError','Erro ao atualizar registro.');
            }
        } catch (error) {
        ...
        try {
            if (!this.eof) {
                ...
                if (status === 200 && data) {
                    ...
                    this.bof = false; 
                    this.eof = true;
                    this.showMessage('mtError',`Erro ao obter próximo registro: ${data.message}`);
                    this.updateCommands(); 
                    return false;
                }
            }
        ...
        try {
            if (!this.bof) {
                ...
                if (status === 200 && data) {                    
                    ...
                    this.bof = true;  
                    this.eof = false;
                    this.showMessage('mtError',`Erro ao obter registro anterior: ${data.message}`);
                    this.updateCommands(); 
                    return false; 
                }
            } 
    ...
    async goEof() {
        try {
            ...
            if (status === 200) {
                ...
                this.showMessage('mtError',`Erro ao ir para o último registro: ${data.message}`);
                return false; // Retorna false em caso de falha
            }
        } catch (error) {
