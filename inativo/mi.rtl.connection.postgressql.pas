...
unit mi.rtl.connection.postgresSQL;
...
type
  ...
  TDM_Connections = class(TDataModule)
    ...
    {: O método **@name** checa se o arquivo **config.ini** existe, se existir,
       ler os parãmetros de conexão do arquivo **config.ini**, se não existir
       pede para a pessoa que executou o programa informar parâmetros para
       conexão e em seguida salva no arquivo **config.ini**.
    ...
    procedure SQLConnector1AfterDisconnect(Sender: TObject);
    procedure SQLConnector1BeforeConnect(Sender: TObject);
    procedure SQLConnector1BeforeDisconnect(Sender: TObject);
    procedure SQLConnector1Log(Sender: TSQLConnection; EventType: TDBEventType;
      const Msg: String);
  ...
  public
    ...
    { O método **@name** conecta-se ao banco de dados se os parâmetros do usuário
       postgres estiverem corretos, se não tiverem, retorna false e não pode tentar
       acessar o banco.
    }
//    function UserPostgresConected:Boolean;
...
function TestArrayOfConst(template:string; Args: array of const):integer;

  // Exemplo de uso: TestArrayOfConst('template',['Paulo',1,2.5]);
Var I : longint;
...
//function TDM_Connections.UserPostgresConected:Boolean;
//  var
...
//                 end;
//
//         until Result or (Result_GetForm_Connections <> TObjectss.MI_MsgBox.mrOK);
//    end;
//end;
//
procedure TDM_Connections.DataModuleCreate(Sender: TObject);
begin
//  UserPostgresConected;
end;

procedure TDM_Connections.SQLConnector1AfterConnect(Sender: TObject);
begin

end;

procedure TDM_Connections.SQLConnector1AfterDisconnect(Sender: TObject);
begin

end;

procedure TDM_Connections.SQLConnector1BeforeConnect(Sender: TObject);
begin

end;

procedure TDM_Connections.SQLConnector1BeforeDisconnect(Sender: TObject);
begin

end;

procedure TDM_Connections.SQLConnector1Log(Sender: TSQLConnection;
